# Troubleshooting — IT-Stack FREEIPA
...
docker compose logs --tail=50 freeipa
...
1. Check logs: docker compose logs freeipa
2. Verify environment variables are set correctly
3. Check database connectivity: pg_isready -h lab-db1 -p 5432
4. Verify port is not already in use: ss -tlnp | grep 389
...
```bash
# Test connectivity
psql -h lab-db1 -U freeipa_user -d freeipa_db -c '\conninfo'
...
1. Check resource usage: docker stats it-stack-freeipa
...
| Log | Path |
|-----|------|
| Application | docker compose logs freeipa |
